...
/** GET /api/v1/admin/queues/claims — pending ownership claims, oldest first. */
export async function GET() {
  ...
  const items = await db.ownerClaim.findMany({
    where: { status: "PENDING" },
    orderBy: { createdAt: "asc" },
    take: 100,
    include: {
      user: { select: { id: true, name: true, telegramId: true } },
      restaurant: { select: { slug: true, phone: true, translations: { where: { locale: "uz" }, select: { name: true } } } },
    },
  });
